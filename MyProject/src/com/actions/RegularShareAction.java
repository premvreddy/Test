...
public class RegularShareAction extends ActionSupport implements ModelDriven<RegularInfoDto>, ServletRequestAware {
	...
	private RegularShareService shareService = null;
	...
	public String execute() {
		...
		try {
			shareService = new RegularShareServiceImpl();
			RegularShareEntity rshareInfo = shareService.addUser(bankname, bankcode, memberid, memberid1, accountno, userName,
					password, name, gender, age, phone, res, cell, qualification, caste, occupation, address, paddress,
					fsalary, fbusiness, frent, fsource, afhname, idoccupation, idaddress, omembership, ipapplicant,
					membersociety, nmname, relation, nmaddress, name1, accountno1, name2, accountno2, shareamount,
					openingbalance, obtype, subgroup, maingroup, accountopendate, dob, pannumber, aadhaarnumber, flatno,
					caddress2, landmark, city, district, state, cpin, pflatno, paddress2, plandmark, pcity, pdistrict,
					pstate, ppin);
			System.out.println("inside execute action "+rshareInfo);
			if (rshareInfo == null) {
				resultString = ERROR;
	...
	public String getRegularList() {
		...
		try {
			shareService = new RegularShareServiceImpl();
			List<RegularShareEntity> info = shareService.getRegularInfos(reqDto);
	...
	public String addUser() {
		...
		try {
			...
			shareService = new RegularShareServiceImpl();
			shareService.addUser(bankname, bankcode, memberid,memberid1, accountno, userName, password, name, gender, age, phone,
					res, cell, qualification, caste, occupation, address, paddress, fsalary, fbusiness, frent, fsource,
					afhname, idoccupation, idaddress, omembership, ipapplicant, membersociety, nmname, relation,
					nmaddress, name1, accountno1, name2, accountno2, shareamount, openingbalance, obtype, subgroup,
					maingroup, accountopendate, dob, pannumber, aadhaarnumber, flatno, caddress2, landmark, city,
					district, state, cpin, pflatno, paddress2, plandmark, pcity, pdistrict, pstate, ppin);
System.out.println("inside add user acion"+shareService);
		} catch (HibernateException e) {
			resultString = ERROR;
			if (e.getCause() == null) {
				errorString = e.getMessage();
	...
	public String memberList() {
		...
		try {
			if (((String) session.getAttribute("timeout")) == null) {
				...
				System.out.println("inside member list action"+memberidi);
				List<Integer> memberid1 = shareService.getmember();
				Integer memberidi1 = memberid.get(memberid1.size() - 1);
				session.setAttribute("membersize", memberidi1);
				// session.setAttribute("rlist", id.size());
				// session.setAttribute("memberSize", list.size());
				map.put("list", jss.serialize(memberidi1));
				System.out.println("inside member list action"+memberidi1);
	...
	public String updateRegular() {

		JSONSerializer jss = new JSONSerializer();
		try {

			shareService = new RegularShareServiceImpl();
			shareService.updateRegular(slno, bankname, bankcode, memberid,memberid1, accountno, userName, password, name, gender,
					age, phone, res, cell, qualification, caste, occupation, address, paddress, fsalary, fbusiness,
					frent, fsource, afhname, idoccupation, idaddress, omembership, ipapplicant, membersociety, nmname,
					relation, nmaddress, name1, accountno1, name2, accountno2, shareamount, openingbalance, obtype,
					subgroup, maingroup);
	...
	public String DeleteRegularShare() {
		session = request.getSession();
		try {

			shareService = new RegularShareServiceImpl();
			shareService.deleteregularshare(slno, RegularShareEntity.class);
